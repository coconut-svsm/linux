				  ...
				  struct page *src_page, void *opaque)
{
	...
	if (src_page) {
		...
		if (sev_populate_args->type == KVM_SEV_SNP_PAGE_TYPE_VMSA &&
		    (vmsa->vmpl || vmsa->sev_features != sev_plane->vmsa_features)) {
...
static bool sev_es_are_required_ghcb_fields_valid(struct vcpu_svm *svm)
{
	...
	case SVM_VMGEXIT_AP_CREATION: {
		unsigned int request = lower_32_bits(control->exit_info_1);

		request &= ~SVM_VMGEXIT_AP_VMPL_MASK;
		return kvm_ghcb_rax_is_valid(svm) || request == SVM_VMGEXIT_AP_DESTROY;
	}
...
int sev_handle_vmgexit(struct kvm_vcpu *vcpu)
{
	...
	case SVM_VMGEXIT_AP_CREATION: {
		int r = sev_snp_ap_creation(vcpu);
